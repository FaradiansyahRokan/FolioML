  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-all duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-rose-50 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-indigo-400 to-rose-400 flex items-center justify-center">
              <svg className="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 1116.65 16.65z" />
              </svg>
            </div>
            <div>
              <h2 className="text-sm font-bold text-stone-800">Web Search & Preview</h2>
              <p className="text-[10px] text-stone-400">Pilih artikel yang ingin diimpor ke Corpus</p>
            </div>
          </div>
          <button onClick={onClose} className="h-7 w-7 rounded-lg flex items-center justify-center text-stone-400 hover:bg-slate-200 hover:text-stone-700 transition-colors">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        {/* Search bar */}
        <form onSubmit={handleSearch} className="px-6 py-3 border-b border-slate-100 flex-shrink-0 bg-white">
          <div className="flex gap-2">
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              disabled={loading || importingUrls.size > 0}
              className="w-36 text-sm px-3 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:border-indigo-400 bg-zinc-50 text-zinc-700 disabled:opacity-50"
            >
            ...
            <input
              ...
              disabled={loading || importingUrls.size > 0}
              className="flex-1 text-sm px-4 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:opacity-50 bg-white"
            />
            <button
              type="submit"
              disabled={!query.trim() || loading || importingUrls.size > 0}
              className="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm"
            >
              {loading ? (
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5 bg-slate-50/50">
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm text-stone-500 font-medium">Mencari artikel terpercaya...</p>
              <p className="text-xs text-stone-400">Mengambil hasil terbaik dari web</p>
            </div>
          )}
            ...
            <>
              <div className="flex items-center justify-between px-1 pb-1">
                <p className="text-[11px] font-semibold text-stone-400">
                  {results.length} hasil untuk <span className="text-indigo-600">"{searchedQuery}"</span>
                </p>
                <button 
                  onClick={handleSelectAll}
                  disabled={importingUrls.size > 0}
                  className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                >
                ...
                return (
                  <div
                    ...
                    className={`rounded-xl border p-4 transition-all relative ${
                      ...
                      : isSelected ? "bg-indigo-50/50 border-indigo-400 shadow-sm ring-1 ring-indigo-200"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 mb-2 pr-8">
                      ...
                      />
                      <span className="text-xs font-semibold text-stone-500 truncate">{r.source_domain}</span>
                    </div>

                    {/* Title */}
                    <h3 className={`text-sm font-bold leading-snug mb-1.5 line-clamp-2 pr-8 ${isSelected ? "text-indigo-900" : "text-stone-800"}`}>
                      {r.title}
                    </h3>

                    {/* Snippet */}
                    <p className="text-xs text-stone-500 leading-relaxed line-clamp-3 mb-3">{r.snippet}</p>

                    {/* URL + Action Status */}
                    <div className="flex items-center justify-between gap-2" onClick={e => e.stopPropagation()}>
                      <a href={r.url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-indigo-500 hover:text-indigo-700 hover:underline truncate max-w-[60%] font-medium">
                        {r.url}
                      </a>
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-white flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-2.5">
            <button 
              onClick={onClose} 
              disabled={importingUrls.size > 0}
              className="text-xs font-semibold px-4 py-2.5 rounded-xl border border-slate-300 text-stone-600 hover:bg-slate-100 transition-colors disabled:opacity-50"
            >
              Tutup
            </button>
            <button 
              ...
              className="flex items-center gap-2 text-xs font-bold px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-indigo-200"
            >
